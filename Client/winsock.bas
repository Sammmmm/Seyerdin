...
'any questions about this file may be posted to alt.winsock.programming'   Topaz..
...
Function ConnectSock(ByVal host$, ByVal Port&, retIpPort$, ByVal HWndToMsg&, ByVal Async%) As Long
    ...
    With LingerType
        ...
        .l_linger = 0
    End With
    If setsockopt(s, SOL_SOCKET, SO_LINGER, LingerType, 4) = SOCKET_ERROR Then
        If s > 0 Then
            dummy = closesocket(s)
        End If
        ConnectSock = INVALID_SOCKET
...
Function GetPeerAddress(ByVal s&) As String
    ...
    End If
    If Err Then GetPeerAddress = vbNullString
End Function
'this function should work on 16 and 32 bit systems
